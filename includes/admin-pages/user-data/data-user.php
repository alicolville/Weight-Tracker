...
<?php if(!empty($_GET['user-preference-saved'])) : ?>
	<div class="notice notice-success"><p><?php echo esc_html__('The preferences for this user have been saved.', WE_LS_SLUG); ?></p></div>
<?php endif; ?>

<?php if(!empty($_GET['deletecache'])) :
		ws_ls_cache_user_delete( $user_id );
    ?>
    <div class="notice notice-success"><p><?php echo esc_html__('The cache for this user has been deleted.', WE_LS_SLUG); ?></p></div>
<?php endif; ?>
...
?>
	<div class="postbox <?php ws_ls_postbox_classes( 'chart', 'ws-ls-user-data-one' ); ?>" id="chart">
		<?php ws_ls_postbox_header( [ 'title' => esc_html__( 'Chart', WE_LS_SLUG ), 'postbox-id' => 'chart', 'postbox-col' => 'ws-ls-user-data-one' ] ); ?>
		<div class="inside">
			<?php
			...
			if( true === ws_ls_meta_fields_photo_any_enabled() ) {
				...
				if ( $photo_count >= 1 ) {
					echo ws_ls_photos_shortcode_gallery( [
						'error-message'        => esc_html__( 'No photos could be found for this user.', WE_LS_SLUG ),
						'mode'                 => 'tilesgrid',
						'limit'                => 20,
						'direction'            => 'desc',
						'user-id'              => $user_id,
...
?>
	<div class="postbox <?php ws_ls_postbox_classes( 'macros', 'ws-ls-user-data-one' ); ?>" id="macros">
		  <?php ws_ls_postbox_header( [ 'title' => esc_html__('Macronutrient Calculator', WE_LS_SLUG), 'postbox-id' => 'macros', 'postbox-col' => 'ws-ls-user-data-one' ] ); ?>
		  <div class="inside">
		<?php
...
?>
	<div class="postbox <?php ws_ls_postbox_classes( 'daily-calories', 'ws-ls-user-data-one' ); ?>" id="daily-calories">
		  <?php ws_ls_postbox_header( [ 'title' => esc_html__('Daily calorie needs', WE_LS_SLUG), 'postbox-id' => 'daily-calories', 'postbox-col' => 'ws-ls-user-data-one' ] ); ?>
		  <div class="inside">
		<?php
...
?>
	<div class="postbox <?php ws_ls_postbox_classes( 'awards', 'ws-ls-user-data-one' ); ?>" id="awards">
		  <?php ws_ls_postbox_header( [ 'title' => esc_html__('Awards', WE_LS_SLUG), 'postbox-id' => 'awards', 'postbox-col' => 'ws-ls-user-data-one' ] ); ?>
		  <div class="inside">
		<?php
			...
			if( true === ws_ls_has_a_valid_pro_plus_license() ) {
				...
				if ( false === empty( $awards ) ) {
					...
					printf( '<a class="button-secondary delete-awards-confirm" href="%1$s">
							...
							esc_html__( 'Delete ALL existing awards', WE_LS_SLUG )
					);

					ws_ls_create_dialog_jquery_code( 	esc_html__('Are you sure you?', WE_LS_SLUG ),
												esc_html__('Are you sure you wish to remove all awards for this user? Note, when the user next enters a weight the awards will be re-calculated.', WE_LS_SLUG) . '<br /><br />',
				  ...
				  printf('<p><a href="%s">%s</a> <strong>- %s</strong> - %s. <a href="%s">%s</a>.</p>',
					  ...
					  esc_html__( 'Modify your awards', WE_LS_SLUG )
				  );
			 } ?>
		  </div>
  	</div>
...
?>
	<div class="postbox <?php ws_ls_postbox_classes( 'user-entries', 'ws-ls-user-data-one' ); ?>" id="user-entries">
		<?php ws_ls_postbox_header( [ 'title' => esc_html__( 'Entries for this user', WE_LS_SLUG ), 'postbox-id' => 'user-entries', 'postbox-col' => 'ws-ls-user-data-one' ] ); ?>
		<div class="inside">
			<?php
