...
function ws_ls_set_user_preferences($in_admin_area, $fields = [])
{
    ...
    $db_fields['dob'] = (false === empty($db_fields['dob'])) ? ws_ls_convert_date_to_iso($db_fields['dob'], ($in_admin_area) ? false : $defaults['user_id']) : '0000-00-00 00:00:00';

    // Save Height, if not specified look up.
    if (false !== $db_fields['height']) {
        $db_fields['height'] = ws_ls_validate_height($db_fields['height']);
    } else {
        $db_fields['height'] = ws_ls_get_user_height($defaults['user_id'], false);
    }
    ...
    // Tidy up cache
    ws_ls_delete_cache_for_given_user($defaults['user_id']);
    return $result;
}
